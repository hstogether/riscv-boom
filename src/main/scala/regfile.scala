   ...
   extends RegisterFile(num_registers, num_read_ports, num_write_ports, register_width, bypassable_array)
{
   ...
   //assert( bypassable_array.length == 2 , "byapssable_array.length != 2")
   //assert( io.write_ports.length == 2 , "io.write_ports.length != 2")
   // bypassable_array.length =2 fpu
   // io.write_ports.length =2 fpu
