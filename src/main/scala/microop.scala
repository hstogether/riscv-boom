...
class MicroOp(implicit p: Parameters) extends BoomBundle()(p)
{
   ...
   val dst_rtype        = UInt(width=3)
   val lrs1_rtype       = UInt(width=3)
   val lrs2_rtype       = UInt(width=3)
   val lrs3_rtype       = UInt(width=3)
   val frs3_en          = Bool()
   ...
   val hfp_val          = Bool()             // is a half-precision floating-point instruction (H-extension)? -- Jecy
