...
// The half-precision floating point issue window, regfile, and arithmetic units are all handled here.
...
class HfpPipeline(implicit p: Parameters) extends BoomModule()(p)
{
   ...
   for (eu <- exe_units)
   {
      for (exe_resp <- eu.io.resp)
      {
         ...
         if (!exe_resp.bits.writesToIRF && !eu.has_ihfpu) {
